...
- Respuestas cortas, máximo 3 oraciones. Menos es más.`;
  ...
  try {
    ...
    try {
      ...
      await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${sheetId}/values/A:B:append?valueInputOption=RAW`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${accessToken}`,
          'Content-Type': 'application/json'
        },
        body: JSON.stringify({ values: [[fecha, `Usuario: ${message}`, `Bot: ${reply}`]] })
      });
    } catch (sheetsErr) {
