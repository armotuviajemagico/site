export default async function handler(req, res) {
  ...
  const { message, history, saveConversation } = req.body;
  if (!message && !saveConversation) return res.status(400).json({ error: 'No message provided' });

  // Guardar conversación completa en Google Sheets
  if (saveConversation && history && history.length > 0) {
    try {
      ...
      const signingInput = `${base64Header}.${base64Claim}`;
      ...
      const conversacion = history.map(m => `${m.role === 'user' ? 'Usuario' : 'Bot'}: ${m.content}`).join('\n');

      // Guardar en Sheets
      await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${sheetId}/values/A:C:append?valueInputOption=RAW`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${accessToken}`,
          'Content-Type': 'application/json'
        },
        body: JSON.stringify({ values: [[fecha, conversacion]] })
      });
